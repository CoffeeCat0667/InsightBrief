...
- **抓取任务统计概览**：统计信息新增「概览」tab，展示全部文章总数 + Top 20 来源横向柱状图，与简报概览风格一致。
- **未简报文章计数**：简报任务「生成所有未简报文章」复选框旁实时显示未生成简报的文章数量。
- **简报按源统计下拉框**：简报按源统计的输入框改为下拉选择器，与抓取任务按源统计 UI 一致。

### 修复

- **简报概览 422 错误**：前端调用 `/api/briefs/stats-overview` 被 FastAPI 匹配到 `/{brief_id}: int` 路由导致类型校验失败；修正为 `/api/brief-tasks/stats-overview`。
- **简报按源统计输入框**：从手动输入新闻源名称改为下拉选择器，避免名称拼写错误。

### 变更

- 版本号升至 `0.2.6`（README badge、CHANGELOG 标题）。

### 验证

- Python `compileall`、全量 JS `node --check`、`git diff --check`。
- 统计概览端点 `GET /api/articles/stats-overview` 实测返回 `total` + `by_source`。
- 未简报文章计数端点 `GET /api/brief-tasks/pending-count` 实测返回正确数量。
- 简报概览/按天/按源三模式前端渲染验证，无 422 错误。
...
- **简报统计信息**（简报任务选项卡）：
  ...
  - 按源统计：选择新闻源 + 时间范围，横向柱状图展示每日简报数量及日均值。
